...
type FileOptions = {
  NSFileProtectionKey?: string; // IOS only
  encrypted?: boolean; // UWP only
  passphrase?: string; // UWP only
  iv?: string; // UWP only, this should be base64 encoded, this will be autogenerated if it is not supplied.
  creationCollisionOption?: "FailIfExists" | "OpenIfExists" | "ReplaceExisting" | "GenerateUniqueName"; // UWP (copyFileToDownloads) only
  overwrite?: Boolean; // UWP (copyFile) only, set to true to allow file overwrites
};
...
var RNFS = {
  ...
  copyFileToDownloads(filepath: string, filename: string, options: FileOptions = {}): Promise<string> {
    return RNFSManager.copyFileToDownloads(normalizeFilePath(filepath), filename, options);
  },
  ...
  readAsFloat(filepath: string, length: number = 0, position: number = 0, encodingOrOptions?: any): Promise<string> {
    return RNFSManager.readAsFloat(normalizeFilePath(filepath), length, position).then((contents) => {
      return contents;
    });
  },

  readAsInt8(filepath: string, length: number = 0, position: number = 0, encodingOrOptions?: any): Promise<string> {
    return RNFSManager.readAsInt8(normalizeFilePath(filepath), length, position).then((contents) => {
      return contents;
    });
  },
  ...
  downloadFile(options: DownloadFileOptions, fileOptions: FileOptions = defaultFileOptions): { jobId: number, promise: Promise<DownloadResult> } {
    ...
    return {
      jobId,
      promise: RNFSManager.downloadFile(bridgeOptions, {...defaultFileOptions, ...fileOptions}).then(res => {
